...
def open_app(command):
    if "notepad" in command:
        if os.path.exists("C:\\Windows\\System32\\notepad.exe"):
            speak("Yes Munu, opening Notepad!")  # speak first
            time.sleep(0.2)
        ...
        if os.path.exists(calc_path):
            speak("Yes Munu, opening Calculator!")
            time.sleep(0.2)
...
while True:
    ...
    wake_words = ["chinu", "chew", "chin", "new", "yo chinu", "yo chew", "yo chin"]
    if any(wake_word in text for wake_word in wake_words):
        speak("Yes Mrunmayee taayi")  # Wake word response
